      ...
      commonService.getData(ALL_PORT.Play.url, 'POST',
        ...
        .then(function (response) {
          if (response.Status == 200) {
            ...
            $scope.PlayListFlag=response.Data.PlayListFlag;
          ...
          // }
            if (response.Data && response.Data.Content == null) {
              if ((response.Data.PortalId) && (response.Data.UserId) && (response.Data.CourseId)) {
                //refresh
                if (loadOnce == 0) {
                  commonService.refresh($scope.allPlayInfo.PortalId, $scope.allPlayInfo.UserId, $scope.allPlayInfo.CourseId,$scope.allPlayInfo.PlayRightAuth?true:false,$scope.allPlayInfo.PlayListFlag?$scope.allPlayInfo.CourseId:0);
                  loadOnce = 1;
                }
            ...
            var timer;
            $loading.start('checkFace')
            function checkFace(){
              commonService.getData(ALL_PORT.CheckFace.url,'POST',
                $.extend({},ALL_PORT.CheckFace.data,{id:$scope.Id}))
              .then(function(response){
                if(response.Type !== 0){
                  $loading.finish('checkFace')
                  clearInterval(timer);
                  $scope.loadPlayInfo();
                }
              })
            }
            timer = setInterval(checkFace,2000);
            // window.close();
          }else{
            alert(response.Message);
            window.close();
          ...
          var _sendProcess = function () {
            if (_thePlayer.getPosition()) {
              ...
              commonService.getData(ALL_PORT.SingleProcess.url, 'POST', data)
                .then(function (data) {
                  // $scope.loadPlayInfo();
                  $scope.resultCourseDetail.BrowseScore = data.BrowseScore;
                }, function () {
                  alert("网路异常，将刷新!");
