...
angular.module('luZhouApp')
  .controller('PersonalearningarchivesCtrl', function ($scope, $timeout, $rootScope, $cookieStore, commonService, $location, $loading,$stateParams) {
    // $scope.startDate = '';
    // $scope.endDate = '';
    // $scope.yearHistory = [
    //   {name: '2018', value: ''},
    ...
    // $scope.studyStatisticsUrl = 'http://zhishan.jy365.net/api' + $scope.selectYearValue + '/Page/StudyStatistics';
    ...
    $loading.start('historyMonth');
    $scope.courseClassify=[];
    commonService.getData(ALL_PORT.GetMonth.url,'POST',ALL_PORT.GetMonth.data)
      .then(function(response){
        // console.log($scope.historyMonth)
        $loading.finish('historyMonth');
        // var historyMonth=response.Data;
        $scope.historyMonth=response.Data;
      ...
      $scope.monthList=function(options){
        $loading.start('historydetail');
        commonService.getData(ALL_PORT.GetMonthCourseList.url,'POST',$.extend(params,options))
        .then(function(response){
          $loading.finish('historydetail');
          $scope.monthListdata=response.Data;
        })
      }
      $scope.monthList();
  });
